...
Modified for the assignment3, to manage characters other than hangul characters, such as punctuations or '<', '>', ...  
...
def getInitialJamo(syl):
    ...
    """
    '''
    Modified: check isHangulSyllable(syl) once in the following decodeSyllable
    '''
    # convert to a number
...
def getFinalJamo(syl):
    ...
    """
    '''
    Modified: check isHangulSyllable(syl) once in the following decodeSyllable function
    '''
    # convert to a number
    sylOrd = float(ord(syl))
    # % is the modulo operator. It gets the remainder.
...
def decodeSyllable(syl):
    ...
    '''
    Modified part.
    returns syl itself if it is not a hangul character. 
    '''
