...
if __name__ == '__main__':
    ...
    #take some korean text
    #u'' means unicode string (you must do this or Hangul will cause errors)
    hangulText = u"세계는 안녕하세요!" #"Hello World!"

    #You can read unicode from a file using the following method:
    #import codecs
    #unicodeFile = codecs.open(filename, "r", encoding="utf-8")
    #
    #Just remember the imports go at the top of your file.
    #See http://docs.python.org/2/library/codecs.html#codecs.open and
    #http://stackoverflow.com/a/147756/1369712 for more information.
    
    for ch in hangulText: #split it into characters
        initialJamo, midJamo, finalJamo = decodeSyllable(ch) #get the Jamo

        if initialJamo: #if we have some value for initialJamo
            #note that decodeSyllable() calls getInitialJamo() which calls
            #isHangulSyllabe().
            #Therefore, if we have a real value for initialJamo() then
            #ch must be a valid Hangul Syllable 
            print u"The syllable {} contains the jamo {}, {}, and {}".format(ch, initialJamo, midJamo, finalJamo)
        else:
            print u"'{}' is not a valid Hangul Syllable".format(ch)
